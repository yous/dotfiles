...
"  call Decho("saving options",'~'.expand("<slnum>"))
  ...
  "  call Decho("setting s:nbcd_curpos_".bufnr('%')." to winsaveview()",'~'.expand("<slnum>"))
  if (has("win32") || has("win95") || has("win64") || has("win16"))
...
"   call Decho('edit-a-file: case "handling a file": newdir<'.newdir.'> !~ dirpat<'.dirpat.">",'~'.expand("<slnum>"))
   ...
   "   call Decho("edit-a-file: setting s:rexposn_".bufnr("%")."<".bufname("%")."> to winsaveview",'~'.expand("<slnum>"))
...
"  call Dfunc("s:NetrwOpenFile(islocal=".a:islocal.")")
  ...
  if fname !~ '[/\\]'
   if exists("b:netrw_curdir")
    ...
    " save position for benefit of Rexplore
    let s:rexposn_{bufnr("%")}= winsaveview()
"    call Decho("setting s:rexposn_".bufnr("%")."<".bufname("%")."> to winsaveview",'~'.expand("<slnum>"))
    if b:netrw_curdir =~ '/$'
